...
::: {.callout-tip title="HG comments"}
Just an R tip: you only need one of either `library(skimr)` or `skimr::function()`. The first option loads that package so that you can use all of its functions throughout your script. The second option does not load the entire package for you to use in your script. You need to explicitly call the package and the function any time you want to use them.  
:::
...
::: {.callout-tip title="HG comments"}
Correct, but we need more information on how you know this. For example, you can look at data points that fall outside of 1.5 times the interquartile range from the median.
:::
