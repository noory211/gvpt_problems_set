...
ggplot(gss, aes(x=partyid_3)) + geom_bar(mapping=aes(colour=partyid_3, fill=partyid_3)) + labs(x= "Party Affiliations", y="Frequency", title="Level of Party ID in GSS Data")
...
#the suspected outliers could be the maximum (145) and minimum (80) values above/below the mean or any other numbers that close to these values. We know it from the fact that the median and mean values are not really far from each other and the standard deviation is quite high.
...
party affiliation: categorical (nominal or ordinal such as likert scale could work too - very weak democrat, weak democrat, democrat, strong democrat, very strong democrat, etc.)
