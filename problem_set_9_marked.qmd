...
:::{.callout-tip}
To answer this question, you need to fit the null model (in which the effect of `nonSouth` and `pcthsg` on `vote` is zero). You can then compare the predictive power of this null model to your fuller model. If your fuller model provides significantly more predictive power than the null model, you can reject the null model. 
:::
...
My regression results indicate that all three predictors have a statistically significant effect on Obama's votes. A one percent increase in the proportion of Democrats in any states is associated, on average, with a 53.3 percent increase in Obama's vote share, holding all else constant. 

:::{.callout-tip}
If only! This result should shock you and prompt you to take a closer look at the model. `obama12` is measured as a percentage (0-100%), not proportions (0-1). You find that a one percentage point increase in the percentage of the public that identify as Democrats is associated with a 0.533 percentage point increase in Obama's vote share, on average and holding all else constant. 
:::
